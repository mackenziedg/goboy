...
func PrintInstruction(location uint8, opcode uint8, name string, length, duration, shortDuration uint8) {
	fmt.Printf("Byte %X. %X %s: l: %d, d: %d, sd: %d\n", location, opcode, name, length, duration, shortDuration)
}

func argsToUint16(argBytes [2]uint8) uint16 {
	out := uint16(0)
...
func RunFile(data []byte) {
	...
	for i := uint8(0); i < nBytes; i++ {
		...
		j := Instruction(data[i], i, argBytes)
		i = j
		reader.ReadString('\n')
	}
}

func Instruction(opcode uint8, location uint8, argBytes [2]uint8) uint8 {
	var name string
	var length uint8
	...
	case 0x31:
		...
		length = 3
		duration = 12
		registers16["SP"] = argsToUint16(argBytes)
	case 0x32:
		name = "LD (HL-),A"
		length = 1
		duration = 8
	...
	case 0xCB:
		...
		length = 1
		duration = 4
		length = 2 //Remove once implemented
	case 0xCC:
		name = "CALL Z,a16"
		length = 3
		duration = 24
...
func CBInstruction(opcode uint8, location uint8, argByte uint8) (string, uint8, uint8) {
	var name string
	var length uint8
	var duration uint8
